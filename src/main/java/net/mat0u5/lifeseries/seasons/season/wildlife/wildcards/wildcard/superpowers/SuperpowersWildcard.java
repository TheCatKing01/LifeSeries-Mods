...
private static boolean POWER_STACKING;

public static void reloadFromConfig(WildLifeConfig config) {
    WILDCARD_SUPERPOWERS_DISABLE_INTRO_THEME = config.WILDCARD_SUPERPOWERS_DISABLE_INTRO_THEME.get(config);
    ZOMBIES_HEALTH = config.WILDCARD_SUPERPOWERS_ZOMBIES_HEALTH.get(config);
    POWER_STACKING = config.WILDCARD_CALLBACK_POWER_STACKING.get(config);

    // Load blacklist
    blacklistedPowers = new ArrayList<>();
    String raw = config.WILDCARD_SUPERPOWERS_POWER_BLACKLIST.get(config);
    String[] powers = raw.replace("[","").replace("]","").split(",");
    for (String s : powers) {
        Superpowers sp = Superpowers.fromString(s.trim());
        if (sp != null && sp != Superpowers.NULL) blacklistedPowers.add(sp);
    }
}
...
public void deactivate() {
    if (!POWER_STACKING) resetAllSuperpowers();
    super.deactivate();
}
...
public static void rollRandomSuperpowers(List<ServerPlayer> allPlayers) {
    ...
    if (necroAllowed) {
        ...
        if (alive + dead >= 6) necroChance = (double) dead / alive;
        else {
            implemented.remove(Superpowers.NECROMANCY);
            necroAllowed = false;
        }
    } else implemented.remove(Superpowers.NECROMANCY);

    Collections.shuffle(implemented);

    for (ServerPlayer player : allPlayers) {
        int powersPerPlayer = WildLifeConfig.WILDCARD_SUPERPOWERS_POWERS_PER_PLAYER.get(Main.WILDLIFE_CONFIG);
        int powersPerRoll = WildLifeConfig.WILDCARD_SUPERPOWERS_POWERS_PER_ROLL.get(Main.WILDLIFE_CONFIG);
        ...
        for (int i = 0; i < powersPerRoll && playerSet.size() < powersPerPlayer; i++) {
            ...
            if (power == null && !implemented.isEmpty()) power = implemented.get(i % implemented.size());
...
public static Superpower getSuperpowerInstance(ServerPlayer player) {
    ...
    for (Superpower power : powers) if (power.active) return power;
...
public static Superpowers getSuperpower(ServerPlayer player) {
    ...
    if (instance instanceof Mimicry mimicry) return mimicry.getMimickedPower().getSuperpower();
