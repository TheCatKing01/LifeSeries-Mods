...
import net.mat0u5.lifeseries.seasons.season.wildlife.wildcards.wildcard.superpowers.superpower.Superpowers;
...
public class SuperpowersWildcard extends Wildcard {
    ...
    private static final Map<UUID, Superpowers> assignedSuperpowers = new HashMap<>();
    ...
    public static void resetSuperpower(ServerPlayer player) {
        ...
        Set<Superpower> set = playerSuperpowers.remove(uuid);
        if (set != null) set.forEach(Superpower::turnOff);
    ...
    public static void rollRandomSuperpowers(List<ServerPlayer> players) {
        ...
        for (ServerPlayer player : players) {
            ...
            // Skip voicechat power if player not connected
            if (power == Superpowers.LISTENING && CompatibilityManager.voicechatLoaded() &&
                    !VoicechatMain.isConnectedToSVC(player.getUUID())) {
                pos++;
                power = implemented.get(pos % implemented.size());
            }

            // Assign preassigned powers
            if (assignedSuperpowers.containsKey(player.getUUID())) {
                power = assignedSuperpowers.remove(player.getUUID());
            } else if (shouldIncludeNecromancy && shouldRandomizeNecromancy && player.getRandom().nextDouble() <= necroChance) {
                power = Superpowers.NECROMANCY;
            }

            if (power == Superpowers.NECROMANCY) shouldIncludeNecromancy = false;

            Superpower inst = power.getInstance(player);
            if (inst != null) playerSuperpowers.computeIfAbsent(player.getUUID(), k -> new HashSet<>()).add(inst);
    ...
    public static void rollRandomSuperpowerForPlayer(ServerPlayer player) {
        ...
        Superpowers power = assignedSuperpowers.getOrDefault(player.getUUID(), implemented.get(0));
    ...
    public static boolean hasActivePower(ServerPlayer player, Superpowers type) {
        ...
        for (Superpower p : set) {
            ...
            } else if (p.getSuperpower() == type) return true;
        }
        return false;
    }
    ...
    public static boolean hasActivatedPower(ServerPlayer player, Superpowers type) {
        ...
        for (Superpower p : set) {
            ...
            } else if (p.active && p.getSuperpower() == type) return true;
        }
        return false;
    }
    ...
    public static Superpower getSuperpowerInstance(ServerPlayer player) {
        ...
        for (Superpower p : set) if (p.active) return p;
    ...
    public static Superpowers getSuperpower(ServerPlayer player) {
        ...
        if (inst instanceof Mimicry mimic) return mimic.getMimickedPower().getSuperpower();
