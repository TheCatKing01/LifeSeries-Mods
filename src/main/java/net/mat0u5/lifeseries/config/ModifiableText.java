...
public enum ModifiableText {
    ...
    ,MOD_ERROR_GENERAL("Â§cSomething went wrong")
    ...
    ,MOD_RELOAD("Â§7Reloading the Life Series...")
    ,MOD_DISABLED_ERROR("The Life Series mod is disabled!\nEnable with \"/lifeseries enable\"")
    ,TRANSCRIPT_COPY("Â§7Click {}Â§7 to copy the session transcript.", List.of("ClickHere"))
    ,CONFIG_UPDATED("Â§7Config has been successfully updated.")
    ,CONFIG_GUI_OPENING("Â§7Opening the config GUI...")
    ,CONFIG_SET("Updated '{}' in the config.", List.of("key"))
    ,COUNTDOWN_COLOR_3("Â§a3")
    ,COUNTDOWN_COLOR_2("Â§e2")
    ,COUNTDOWN_COLOR_1("Â§c1")
    ,COUNTDOWN_RED_3("Â§c3")
    ,COUNTDOWN_RED_2("Â§c2")
    ,COUNTDOWN_RED_1("Â§c1")
    ,COUNTDOWN_GREEN_3("Â§a3")
    ,COUNTDOWN_GREEN_2("Â§a2")
    ,COUNTDOWN_GREEN_1("Â§a1")

    ,SEASON_COMMANDS_ADMIN(Formatted.LOOSELY_STYLED, "Â§7{} commands: Â§r{}", List.of("season", "commands"))
    ,SEASON_COMMANDS(Formatted.LOOSELY_STYLED, "Â§7{} non-admin commands: Â§r{}", List.of("season", "commands"))
    ...
    ,SEASON_SELECT_WARNING("Â§7WARNING: you have already selected a season, changing it might cause some saved data to be lost (lives, ...)\nÂ§7If you are sure, use 'Â§f/lifeseries setSeries <season> confirmÂ§7'")
    ,SEASON_CHANGE(Formatted.LOOSELY_STYLED,"Â§aSuccessfully changed the season to {}.", List.of("season"))
    ,SEASON_CHANGING("Â§7Changing the season to {}Â§7...", List.of("season"))
    ,SEASON_CHANGED(Formatted.LOOSELY_STYLED,"Â§aSuccessfully changed the season to {}", List.of("season"))
    ,SEASON_GET("Current season: {}", List.of("season"))
    ,SEASON_KILL_GAINLIFE("{}Â§7 gained a life for killing {}.", List.of("Killer", "Victim"))
    ,SEASON_KILL_UNJUSTIFIED("Â§c [Unjustified Kill?] {}Â§7 was killed by {}", List.of("Killer", "Victim"))
    ,SEASON_SELECTION_GUI("Â§7Opening the season selection GUI...")

    ,SESSION_STARTED(Formatted.LOOSELY_STYLED,"Â§6Session started! Â§7[{}]\nÂ§f/session timer showDisplayÂ§7 - toggles a session timer on your screen.", List.of("session length"))
    ,SESSION_ACTION_ENTRY(Formatted.LOOSELY_STYLED,"Â§7- {}", List.of("action name"))
    ,SESSION_ACTION_ENTRY_LONG(Formatted.LOOSELY_STYLED,"Â§7- {} Â§f[{}]", List.of("action name", "trigger time"))
    ...
    ,SESSION_ERROR_START("Â§cThe session has not started")
    ,SESSION_START_PROMPT("\nUse Â§b'/session timer set <time>'Â§f to set the desired session time.\nAfter that, use Â§b'/session start'Â§f to start the session.")
    ...
    ,SESSION_STARTING("Â§7Starting session...")
    ,SESSION_STOPPING("Â§7Stopping session...")
    ,SESSION_PAUSING("Â§7Pausing session...")
    ,SESSION_UNPAUSING("Â§7Unpausing session...")
    ,SESSION_ENDED("Â§6The session has ended!")
    ,SESSION_PAUSED("Â§6Session paused!")
    ,SESSION_UNPAUSED("Â§6Session unpaused!")
    ,SESSION_WARNING_5MIN("Â§6Session ends in 5 minutes!")
    ,SESSION_WARNING_30MIN("Â§6Session ends in 30 minutes!")
    ,SESSION_START_TITLE("Â§aThe timer has begun!")
    ,SESSION_PAUSE_QUEUE_RESET("Reset all queued pauses")
    ,SESSION_TIMER_DISPLAY_NOTSTARTED("Â§7Session has not started")
    ,SESSION_TIMER_DISPLAY_PAUSE("Â§7Session has been paused")
    ,SESSION_TIMER_DISPLAY_END("Â§7Session has ended")
    ,SESSION_TIMER_DISPLAY("Â§7{}", List.of("time"))
    ,SESSION_ACTIONS("Â§7Queued session actions:")
    ...
    ,TICK_FREEZE("Â§7The game is frozen")
    ,TICK_UNFREEZE("Â§7The game is no longer frozen.")
    ...
    ,LIVES_RELOADING("Â§7Reloading lives...")
    ...
    ,LIVES_RANDOMIZE_SINGLE("Â§7Assigning random lives to {}Â§7...", List.of("Player"))
    ,LIVES_RANDOMIZE_MULTIPLE("Â§7Assigning random lives to {}Â§7 targets...", List.of("number of targets"))
    ,LIVES_RANDOMIZE_RESULT("{}Â§a {}.", List.of("amount", "life/lives"))
    ,LIVES_RANDOMIZE_TITLE("Â§7You will have...")
    ,LIVES_SHOW_DIFF_GAIN(Formatted.LOOSELY_STYLED, " Â§a[+{}]", List.of("amount"))
    ,LIVES_SHOW_DIFF_LOSS(Formatted.LOOSELY_STYLED, " Â§c[-{}]", List.of("amount"))
    ...
    ,GIVELIFE_DOUBLELIFE_INFO("Â§7Your soulmate must accept your request to give a life to this player.")

    ,WATCHER_JOIN("Â§7Â§nYou are now a Watcher.\n\nÂ§7Watchers are players that are online, but are not affected by most season mechanics. They can only observe - this is very useful for spectators and for admins.")
    ,WATCHER_LEAVE("Â§7You are no longer a Watcher.")
    ...
    ,WATCHER_LIST(Formatted.LOOSELY_STYLED,"Current Watchers: Â§7{}", List.of("Watchers"))
    ,MUTED_WATCHER("Watchers aren't allowed to talk in chat! Admins can change this behavior in the config.")
    ,WATCHER_INFO("Â§7Watchers are players that are online, but are not affected by most season mechanics. They can only observe.\nÂ§7This is very useful for spectators and for admins.")

    ,SOCIETY_INITIATE_REMINDER("Â§7When you are alone, type \"/initiate\"")
    ,SOCIETY_INITIATED_PT1("Â§7You have been chosen to be part of the Â§csecret societyÂ§7.")
    ,SOCIETY_INITIATED_GROUP_PT1(Formatted.LOOSELY_STYLED, "Â§7There {} Â§c{}Â§7 other {}. Find them.", List.of("is/are", "member amount", "member/s"))
    ,SOCIETY_INITIATED_GROUP_PT2(Formatted.LOOSELY_STYLED, "Â§7Together, secretly kill Â§c{}Â§7 other {} by Â§cnon-pvpÂ§7 means.", List.of("kills needed", "Player/s"))
    ,SOCIETY_INITIATED_GROUP_PT3("Â§7Find the other members with the secret word:")
    ,SOCIETY_INITIATED_GROUP_PT4(Formatted.LOOSELY_STYLED, "Â§d\"{}\"", List.of("secret word"))
    ,SOCIETY_INITIATED_ALONE_PT1("Â§7You are alone.")
    ,SOCIETY_INITIATED_ALONE_PT2(Formatted.LOOSELY_STYLED, "Â§7Secretly kill Â§c{}Â§7 other {} by Â§cnon-pvpÂ§7 means.", List.of("kills needed", "Player/s"))
    ,SOCIETY_INITIATED_PT2("Â§7Type \"/society success\" when you complete your goal.")
    ,SOCIETY_INITIATED_PT3("Â§7Don't tell anyone else about the society.")
    ,SOCIETY_INITIATED_PT4("Â§7If you fail...")
    ,SOCIETY_INITIATED_PUNISHMENT(Formatted.LOOSELY_STYLED, "Â§7Type \"/society fail\", and you all lose Â§c{} {}Â§7.", List.of("lives lost", "life/lives"))
    ,SOCIETY_NOTICE_ADDED("Â§c [NOTICE] You are now a Secret Society member!")
    ,SOCIETY_NOTICE_REMOVED("Â§c [NOTICE] You are no longer a Secret Society member!")
    ...
    ,SOCIETY_MEMBERS(Formatted.LOOSELY_STYLED,"Secret Society Members: Â§7{}", List.of("members"))
    ...
    ,SOCIETY_START_PROMPT("Â§7Use '/society begin' or '/society begin <secret_word>' to start.")
    ,SOCIETY_STARTING("Â§7Starting the Secret Society...")
    ,SOCIETY_ENDING("Â§7Ending the Secret Society...")
    ,SOCIETY_MEMBER_ERROR_SELF("You are not a member of the Secret Society")
    ,SOCIETY_MEMBER_ERROR_FAILBYPASS("Â§7Use the Â§f\"/society fail Â§lconfirm\"Â§lÂ§7 command to bypass this")
    ,SOCIETY_MEMBER_ERROR_SUCCESSBYPASS("Â§7Use the Â§f\"/society success Â§lconfirm\"Â§lÂ§7 command to bypass this")
    ...
    ,SOCIETY_NOT_ENDED("Â§c The Secret Society has not been ended by any Member!\nÂ§c Run \"/society members list\" to see the Members.")
    ,SOCIETY_CALLS_PT1("Â§cThe Society calls")
    ,SOCIETY_CALLS_PT2("Â§cThe Society calls.")
    ,SOCIETY_CALLS_PT3("Â§cThe Society calls..")
    ,SOCIETY_CALLS_PT4("Â§cThe Society calls...")
    ,SOCIETY_CALLS_PT5_TITLE("")
    ,SOCIETY_CALLS_PT5_SUBTITLE("Â§cTake yourself somewhere quiet")
    ,SOCIETY_END_SUCCESS_PT1_TITLE("")
    ,SOCIETY_END_SUCCESS_PT2_TITLE("")
    ,SOCIETY_END_SUCCESS_PT3_TITLE("")
    ,SOCIETY_END_SUCCESS_PT1_SUBTITLE("Â§aThe Society is pleased")
    ,SOCIETY_END_SUCCESS_PT2_SUBTITLE("Â§aYou will not be punished")
    ,SOCIETY_END_SUCCESS_PT3_SUBTITLE("Â§cYou are still sworn to secrecy")
    ,SOCIETY_END_FAIL_PT1_TITLE("")
    ,SOCIETY_END_FAIL_PT2_TITLE("")
    ,SOCIETY_END_FAIL_PT3_TITLE("")
    ,SOCIETY_END_FAIL_PT1_SUBTITLE("Â§cThe Society is displeased")
    ,SOCIETY_END_FAIL_PT2_SUBTITLE("Â§cYou will be punished")
    ,SOCIETY_END_FAIL_PT3_SUBTITLE("Â§cYou are still sworn to secrecy")

    ,SUBIN_END_NOTIFY(Formatted.LOOSELY_STYLED, "Â§6You are no longer subbing in for {}", List.of("Player"))
    ,SUBIN_END_OTHER(Formatted.LOOSELY_STYLED, "Â§6{} is no longer subbing in for you", List.of("Player"))
    ...
    ,SUBIN_LIST_ENTRY(Formatted.LOOSELY_STYLED," Â§7{} is subbinng in for {}", List.of("Player", "Subin"))
    ...
    ,SUBIN_CURRENT("Â§7Current sub ins:")
    ...
    ,CLAIMKILL("{}Â§7 claims credit for {}Â§7's death.", List.of("Killer", "Victim"))
    ,CLAIMKILL_VALIDATE("Â§7Click {}Â§7 to accept the claim if you think it's valid.", List.of("ClickHere"))
    ,CLAIMKILL_ACCEPT("{}Â§7's kill claim on {}Â§7 was accepted.", List.of("Killer", "Victim"))
    ...
    ,LIFESKINS_RELOAD("Â§7Reloading Life Skins...")
    ,LIFESKINS_LIST_EMPTY("There are no Life Skins currently loaded")
    ,LIFESKINS_LIST("Â§7Loaded Life Skins for life boundaries:")
    ,LIFESKINS_LIST_PERSON(Formatted.LOOSELY_STYLED, "Â§f {}: Â§7{}", List.of("Holder", "skins"))
    ,LIFESKINS_INFO("Â§fClick {}Â§f to open the Life Skins info page on the Wiki.", List.of("ClickHere"))
    ...
    ,BOOGEYMAN_MESSAGE("Â§7You are the Boogeyman. You must by any means necessary kill a Â§2dark greenÂ§7, Â§agreenÂ§7 or Â§eyellowÂ§7 name by direct action to be cured of the curse. If you fail, you will become a Â§cred nameÂ§7. All loyalties and friendships are removed while you are the Boogeyman.")
    ,BOOGEYMAN_NOTICE_ADDED("Â§c [NOTICE] You are now a Boogeyman!")
    ,BOOGEYMAN_NOTICE_REMOVED("Â§c [NOTICE] You are no longer a Boogeyman!")
    ,BOOGEYMAN_NOTICE_RESET("Â§c [NOTICE] Your Boogeyman  fail/cure status has been reset")
    ,BOOGEYMAN_KILLS_REQUIRED("Â§7You need {} {}Â§7 to be cured of the curse.", List.of("kills amount", "kill/kills"))
    ,BOOGEYMAN_LATEJOIN("Â§cSince you were not present when the Boogeyman was being chosen, your chance to become the Boogeyman is now. Good luck!")
    ,BOOGEYMAN_LIST("Current Boogeymen: {}", List.of("list"))
    ,BOOGEYMAN_FAIL_NOTICE("Â§cYou only have 5 minutes left to kill someone as the Boogeyman before you fail!")
    ,BOOGEYMAN_FAIL_SELFFAIL("{}Â§7 voluntarily failed themselves as the Boogeyman. They have been consumed by the curse.", List.of("Player"))
    ,BOOGEYMAN_FAIL_OTHER_SINGLE("Â§7Failing Boogeyman for {}Â§7...", List.of("Player"))
    ,BOOGEYMAN_FAIL_OTHER_MULTIPLE("Â§7Failing Boogeyman for {} targetsÂ§7...", List.of("number of targets"))
    ,BOOGEYMAN_RESET_SINGLE("Â§7Resetting Boogeyman cure/failure for {}Â§7...", List.of("Player"))
    ,BOOGEYMAN_RESET_MULTIPLE("Â§7Resetting Boogeyman cure/failure for {} targetsÂ§7...", List.of("number of targets"))
    ,BOOGEYMAN_CURE_SINGLE("Â§7Curing {}Â§7...", List.of("Player"))
    ,BOOGEYMAN_CURE_MULTIPLE("Â§7Curing {} targetsÂ§7...", List.of("number of targets"))
    ...
    ,BOOGEYMAN_CURE("{}Â§7 is cured of the Boogeyman curse!", List.of("Player"))
    ,BOOGEYMAN_CURE_GAINLIFE("{}Â§7 is cured of the Boogeyman curse and gained a life for succeeding!", List.of("Player"))
    ,BOOGEYMAN_FAIL("{}Â§7 failed to kill a player while being the Â§cBoogeymanÂ§7. They have been dropped to their Â§cLast LifeÂ§7.", List.of("Player"))
    ,BOOGEYMAN_FAIL_ADVANCEDDEATH("{}Â§7 failed to kill a player while being the Â§cBoogeymanÂ§7. They have been consumed by the curse.", List.of("Player"))
    ,BOOGEYMAN_FAIL_NOTIFY_TITLE("Â§cYou have failed.")
    ,BOOGEYMAN_FAIL_ADVANCEDDEATH_NOTIFY_TITLE("Â§cThe curse consumes you..")
    ...
    ,BOOGEYMAN_ALREADY_FAILED("Â§7You were the Boogeyman, but you have already Â§cfailedÂ§7.")
    ,BOOGEYMAN_ALREADY_CURED("Â§7You were the Boogeyman, and you have already been Â§acuredÂ§7.")
    ,BOOGEYMAN_IS("Â§cYou are the Boogeyman.")
    ,BOOGEYMAN_SELFFAIL_WARNING("Warning: This will cause you to fail as the Boogeyman\nRun \"/boogeyman selfFail Â§lconfirmÂ§r\" to confirm this action.")
    ,BOOGEYMAN_SELFFAIL("Â§7Failing as the Boogeyman...")
    ,BOOGEYMAN_CLEAR("All Boogeymen have been cleared")
    ,BOOGEYMAN_RANDOMIZE("Â§7Choosing random Boogeymen...")
    ,BOOGEYMAN_NOTICE_5MIN("Â§cThe Boogeyman is being chosen in 5 minutes.")
    ,BOOGEYMAN_NOTICE_1MIN("Â§cThe Boogeyman is being chosen in 1 minute.")
    ,BOOGEYMAN_CHOSEN("Â§cA new boogeyman is about to be chosen.")
    ,BOOGEYMAN_CHOSEN_NEW("Â§cThe Boogeyman is about to be chosen.")
    ,BOOGEYMAN_ROLL("Â§eYou are...")
    ,BOOGEYMAN_ROLL_BOOGEY("Â§cThe Boogeyman.")
    ,BOOGEYMAN_ROLL_NORMAL("Â§aNOT the Boogeyman.")
    ,BOOGEYMAN_CURE_TITLE("Â§aYou are cured!")
    ,BOOGEYMAN_FAIL_ADVANCEDDEATH_FINISH_PT1("Â§cYour lives are taken...")
    ,BOOGEYMAN_FAIL_ADVANCEDDEATH_FINISH_PT2("Â§c...Now take theirs.")


    ,DOUBLELIFE_TELEPORT(Seasons.DOUBLE_LIFE, "Â§6Woosh!")
    ,DOUBLELIFE_TELEPORT_SUCCESS(Seasons.DOUBLE_LIFE, "Randomly distributed players.")
    ,DOUBLELIFE_SOULMATE_TITLE_UNKNOWN(Seasons.DOUBLE_LIFE, "Â§a????")
    ...
    ,DOUBLELIFE_SOULMATE_ROLLING(Seasons.DOUBLE_LIFE, "Â§7Rolling soulmates...")
    ,DOUBLELIFE_UNPAIRED(Seasons.DOUBLE_LIFE, "[Double Life] {} was not paired with anyone.", List.of("Player"))
    ,DOUBLELIFE_SOULMATE_TITLE(Seasons.DOUBLE_LIFE, "Â§aYour soulmate is...")
    ,DOUBLELIFE_LASTPAIR_PT1_TITLE(Seasons.DOUBLE_LIFE, "")
    ,DOUBLELIFE_LASTPAIR_PT1_SUBTITLE(Seasons.DOUBLE_LIFE, "Â§aYour fate is your own...")
    ,DOUBLELIFE_LASTPAIR_PT2_TITLE(Seasons.DOUBLE_LIFE, "")
    ,DOUBLELIFE_LASTPAIR_PT2_SUBTITLE(Seasons.DOUBLE_LIFE, "Â§cThere can only be one winner.")
    ...
    ,LIMITEDLIFE_CHANGE_COLOR(Seasons.LIMITED_LIFE, "{}Â§7 is now a {} nameÂ§7.", List.of("Player", "color"))
    ,LIMITEDLIFE_SESSION_DISPLAY_DIVIDER(Seasons.LIMITED_LIFE, "  |  ")

    ,SECRETLIFE_TASK_MISSING(Seasons.SECRET_LIFE, "Â§cYou do not have a secret task book in your inventory.")
    ...
    ,SECRETLIFE_TASK_SHOW(Seasons.SECRET_LIFE, "Â§7Click {}Â§7 to show the task they have.", List.of("ClickHere"))
    ,SECRETLIFE_TASK_SHOW_PAST(Seasons.SECRET_LIFE, "Â§7Click {}Â§7 to see what {}Â§7's task was.", List.of("ClickHere", "Player"))
    ,SECRETLIFE_SECRETKEEPER_INUSE(Seasons.SECRET_LIFE, "Â§cSomeone else is using the Secret Keeper right now.")
    ,SECRETLIFE_TASK_PENDING(Seasons.SECRET_LIFE, "{} wants to succeed their task.", List.of("Player"))
    ,SECRETLIFE_TASK_PENDING_ACCEPT(Seasons.SECRET_LIFE, "Â§7Click {}Â§7 to confirm this action.", List.of("ClickHere"))
    ,SECRETLIFE_TASK_PENDING_NOTIFICATION(Seasons.SECRET_LIFE, "Â§cYour task confirmation needs to be approved by an admin.")
    ,SECRETLIFE_TASK_SUCCEED(Seasons.SECRET_LIFE, "{}Â§a succeeded their task.", List.of("Player"))
    ,SECRETLIFE_TASK_REROLL_HARD_FAIL(Seasons.SECRET_LIFE, "Â§cYou cannot re-roll a Hard task.")
    ,SECRETLIFE_TASK_REROLL_HARD_FAIL_RED(Seasons.SECRET_LIFE, "Â§cYou cannot re-roll a Hard task. If you want your red task instead, click the Fail button.")
    ,SECRETLIFE_TASK_NOT_SUBMITTED(Seasons.SECRET_LIFE, Formatted.LOOSELY_STYLED,"Â§4{}Â§c still {} not submitted / failed a task this session.", List.of("Players", "has/have"))
    ,SECRETLIFE_HEART_GAIN(Seasons.SECRET_LIFE, Formatted.LOOSELY_STYLED,"Â§c+{} {}", List.of("amount", "heart/hearts"))
    ...
    ,SECRETLIFE_TASK_PREASSIGN(Seasons.SECRET_LIFE, "Pre-assigned {}'s task for randomization\nÂ§7They will be given the task book once you / the game rolls the tasks", List.of("Player"))
    ...
    ,SECRETLIFE_TASK_SUCCESS_SINGLE(Seasons.SECRET_LIFE, "Â§7Succeeding task for {}Â§7...", List.of("Player"))
    ,SECRETLIFE_TASK_SUCCESS_MULTIPLE(Seasons.SECRET_LIFE, "Â§7Succeeding task for {}Â§7 targets...", List.of("number of targets"))
    ,SECRETLIFE_TASK_FAIL_SINGLE(Seasons.SECRET_LIFE, "Â§7Failing task for {}Â§7...", List.of("Player"))
    ,SECRETLIFE_TASK_FAIL_MULTIPLE(Seasons.SECRET_LIFE, "Â§7Failing task for {}Â§7 targets...", List.of("number of targets"))
    ,SECRETLIFE_TASK_REROLL_SINGLE(Seasons.SECRET_LIFE, "Â§7Rerolling task for {}Â§7...", List.of("Player"))
    ,SECRETLIFE_TASK_REROLL_MULTIPLE(Seasons.SECRET_LIFE, "Â§7Rerolling task for {}Â§7 targets...", List.of("number of targets"))
    ...
    ,SECRETLIFE_TASK_REROLL(Seasons.SECRET_LIFE,"{}Â§7 re-rolled their easy task.", List.of("Player"))
    ,SECRETLIFE_TASK_FAIL(Seasons.SECRET_LIFE,"{}Â§c failed their task.", List.of("Player"))
    ...
    ,SECRETLIFE_TASK_WARNING_5MIN(Seasons.SECRET_LIFE, "Â§7Go submit / fail your secret tasks if you haven't!")
    ,SECRETLIFE_TASK_WARNING_30MIN(Seasons.SECRET_LIFE, "Â§7You better start finishing your secret tasks if you haven't already!")
    ,SECRETLIFE_TASK_TITLE(Seasons.SECRET_LIFE, "Â§cYour secret is...")
    ,SECRETLIFE_TASK_REROLL_PT1(Seasons.SECRET_LIFE, "Â§2Â§lThe reward is more")
    ,SECRETLIFE_TASK_REROLL_PT2(Seasons.SECRET_LIFE, "Â§aÂ§lThe risk is great")
    ,SECRETLIFE_TASK_REROLL_PT3(Seasons.SECRET_LIFE, "Â§eÂ§lLet me open the door")
    ,SECRETLIFE_TASK_REROLL_PT4(Seasons.SECRET_LIFE, "Â§cÂ§lAccept your fate")

    ,WILDLIFE_SNAIL_TEXTURE_INFO(Seasons.WILD_LIFE,"Â§fClick {}Â§f to open the Snail Textures info page on the Wiki.", List.of("ClickHere"))
    ,WILDLIFE_SNAIL_DEFAULT_NAME(Seasons.WILD_LIFE, Formatted.PLAIN,"{}'s Snail", List.of("Player"))
    ,WILDLIFE_SNAIL_NAME_REQUEST(Seasons.WILD_LIFE, "{}Â§7 requests their snail name to be Â§f{}Â§7", List.of("Player", "name"))
    ,WILDLIFE_SNAIL_NAME_REQUEST_PROMPT(Seasons.WILD_LIFE, "Â§7Click {}Â§7 to accept.", List.of("ClickHere"))
    ,WILDLIFE_SNAIL_TEXTURES_LIST(Seasons.WILD_LIFE, "Â§7The following skins have been found: Â§f{}", List.of("list"))
    ...
    ,WILDLIFE_POWER_PLAYERDISGUISE(Seasons.WILD_LIFE, "Copied DNA of {} â€” Press again to disguise", List.of("Player"))
    ...
    ,WILDLIFE_WILDCARD_CHOOSE(Seasons.WILD_LIFE, "The {} wildcard has been selected for this session.\nÂ§7Use the Â§f'/wildcard choose' Â§7 command if you want to change it.", List.of("wildcard"))
    ,WILDLIFE_TRIVIA_RECEIVE_EFFECT(Seasons.WILD_LIFE, Formatted.LOOSELY_STYLED, " Â§aÂ§l+ Â§7{}Â§6 {}", List.of("effect name", "amplifier"))
    ...
    ,WILDLIFE_HUNGER_RANDOMIZE_SUBTITLE(Seasons.WILD_LIFE, "Â§7Food is about to be randomised...")
    ,WILDLIFE_HUNGER_RANDOMIZE_MANUAL(Seasons.WILD_LIFE, "Â§7Randomizing food...")
    ,WILDLIFE_SNAIL_TEXTURES_RELOAD(Seasons.WILD_LIFE, "Â§7Reloading snail textures...")
    ,WILDLIFE_SNAIL_TEXTURES_NONE(Seasons.WILD_LIFE, "Â§7No snail skins have been added yet. Run 'Â§f/snail textures infoÂ§7' to learn how to add them.")
    ,WILDLIFE_WILDCARD_GUI_ERROR(Seasons.WILD_LIFE, "You must have the Life Series mod installed Â§nclient-sideÂ§c to open the wildcard GUI")
    ,WILDLIFE_WILDCARD_GUI_OPEN(Seasons.WILD_LIFE, "Â§7Opening the Wildcard selection GUI...")
    ...
    ,WILDLIFE_WILDCARD_ACTIVATED_NONE(Seasons.WILD_LIFE, "Â§7There are no active Wildcards right now. \nYou will be able to select a Wildcard when you start a session, or you can use 'Â§f/wildcard activate <wildcard>Â§7' to activate a specific Wildcard right now.")
    ,WILDLIFE_TRIVIA_QUESTION_INVALID(Seasons.WILD_LIFE, "Could not find trivia with that question.")
    ,WILDLIFE_SNAIL_INFO(Seasons.WILD_LIFE, "Â§7Use the Â§f'/snail ...'Â§7 command to modify snail names and to get info on how to change snail textures.")
    ,WILDLIFE_TRIVIA_NOTICE_START(Seasons.WILD_LIFE, "Â§7You must start a session for trivia bots to spawn!")
    ,WILDLIFE_TRIVIA_NOTICE(Seasons.WILD_LIFE, "Â§7You can modify the trivia questions in the config files (./config/lifeseries/wildlife/*-trivia)")
    ,WILDLIFE_WILDCARD_WARNING_2MIN(Seasons.WILD_LIFE, "Â§7A Wildcard will be activated in 2 minutes!")
    ,WILDLIFE_WILDCARD_FADED(Seasons.WILD_LIFE, "Â§7A Wildcard has faded...")
    ,WILDLIFE_WILDCARD_DOTS_1(Seasons.WILD_LIFE, "Â§aÂ§l,")
    ,WILDLIFE_WILDCARD_DOTS_2(Seasons.WILD_LIFE, "Â§aÂ§l, Â§eÂ§l,")
    ,WILDLIFE_WILDCARD_DOTS_3(Seasons.WILD_LIFE, "Â§aÂ§l, Â§eÂ§l, Â§cÂ§l,")
    ,WILDLIFE_MAKEITWILD_PT1(Seasons.WILD_LIFE, "Â§7The ending is Â§cyoursÂ§7...")
    ,WILDLIFE_MAKEITWILD_PT2(Seasons.WILD_LIFE, "Â§cMake")
    ,WILDLIFE_MAKEITWILD_PT3(Seasons.WILD_LIFE, "Â§cMake Â§eit")
    ,WILDLIFE_MAKEITWILD_PT4(Seasons.WILD_LIFE, "Â§cMake Â§eit Â§aÂ§lWILD")
    ...
    ,PASTLIFE_SESSION_START(Seasons.PAST_LIFE, "Â§7Past Life session started:\nÂ§7 Type Â§f\"/pastlife boogeyman\"Â§7 to have the Boogeyman in this session.\nÂ§7 Type Â§f\"/pastlife society\"Â§7 to have the Secret Society in this session.\nÂ§7 Or type Â§f\"/pastlife pickRandom\"Â§7 if you want the game to pick randomly.\n")
    ,BOOGEYMAN_PASTLIFE_MESSAGE_PT1(Seasons.PAST_LIFE, "Â§7You are the boogeyman.")
    ,BOOGEYMAN_PASTLIFE_MESSAGE_PT2(Seasons.PAST_LIFE, "Â§7You must by any means necessary kill a Â§agreenÂ§7 or Â§eyellowÂ§7 name\nÂ§7by direct action to be cured of the curse.")
    ,BOOGEYMAN_PASTLIFE_MESSAGE_PT3(Seasons.PAST_LIFE, "Â§7If you fail, you will become a Â§cred nameÂ§7.")
    ,BOOGEYMAN_PASTLIFE_MESSAGE_PT4(Seasons.PAST_LIFE, "Â§7Other players may defend themselves.")
    ,BOOGEYMAN_PASTLIFE_MESSAGE_PT5(Seasons.PAST_LIFE, "Â§7Voluntary sacrifices will not cure the curse.")
    ,BOOGEYMAN_PASTLIFE_MESSAGE_PT6(Seasons.PAST_LIFE, "Â§7You need {} {}Â§7 to be cured of the curse.", List.of("kills amount", "kill/kills"))
    ,PASTLIFE_TWIST_FAIL(Seasons.PAST_LIFE, "Picking failed")
    ,PASTLIFE_TWIST_RANDOM(Seasons.PAST_LIFE, "Â§7Randomly picking the Boogeyman or the Secret Society...")
    ...
    ,NICELIFE_NICELIST_START_TITLE_PT1(Seasons.NICE_LIFE, "Â§aThese players are on...")
    ,NICELIFE_NICELIST_START_TITLE_PT2(Seasons.NICE_LIFE, "Â§aTHE NICE LIST")
    ,NICELIFE_NICELIST_START_INFO_PT1(Seasons.NICE_LIFE, "\n Â§6[Â§e!Â§6]Â§7 At sunset, players on the nice list will vote to give a Â§2non-pinkÂ§7 name a life.\n")
    ,NICELIFE_NICELIST_START_INFO_PT2(Seasons.NICE_LIFE, " Â§6[Â§e!Â§6]Â§7 The majority of the Â§dpinksÂ§7 must vote for the same player for the life to be given.\n")
    ,NICELIFE_NICELIST_START_INFO_PT3(Seasons.NICE_LIFE, " Â§6[Â§e!Â§6]Â§7 Pink names are not allowed to be targeted by any other players, including Â§credsÂ§7.\n")
    ,NICELIFE_NICELIST_START_INFO_PT4(Seasons.NICE_LIFE, " Â§6[Â§e!Â§6]Â§7 You are on the nice list. Type {}Â§7 to choose who you would like to give a life to.\n", List.of("ClickHere"))
    ,NICELIFE_NICELIST_START_INFO_PT5(Seasons.NICE_LIFE, " Â§6[Â§e!Â§6]Â§7 You can change your vote at anytime, but the results will be locked in at sunset.\n")
    ,NICELIFE_NICELIST_REMOVED(Seasons.NICE_LIFE, " Â§6[Â§e!Â§6]Â§7 You are no longer on the Nice List")
    ,NICELIFE_NICELIST_VOTE(Seasons.NICE_LIFE, "\n Â§6[Â§e!Â§6]Â§7 You voted for {}Â§7.\n", List.of("Player"))
    ,NICELIFE_NICELIST_VOTE_REMINDER(Seasons.NICE_LIFE, "\nÂ§7Don't forget to {}Â§7!\n", List.of("ClickHere"))
    ...
    ,NICELIFE_NAUGHTYLIST_START_INFO_PT2(Seasons.NICE_LIFE, " Â§6[Â§e!Â§6]Â§7 People on the Â§cnaughty listÂ§7 have a purple name and can be killed.\n")
    ,NICELIFE_NAUGHTYLIST_START_INFO_PT3(Seasons.NICE_LIFE, " Â§6[Â§e!Â§6]Â§7 They return to their previous colour at sunset. They can defend themselves.\n")
    ...
    ,NICELIFE_TRIVIA_ALL_WRONG_PT1(Seasons.NICE_LIFE, "Â§f<Â§2Â§mTriviaÂ§mÂ§2 Santa BotÂ§f>Â§4 WRONG! WRONG! WRONG! ALL WRONG!")
    ,NICELIFE_TRIVIA_ALL_WRONG_PT2(Seasons.NICE_LIFE, "Â§f<Â§2Â§mTriviaÂ§mÂ§2 Santa BotÂ§f>Â§4 SNOW MUST GO ON!")
    ,NICELIFE_REDWINTER_PT1(Seasons.NICE_LIFE, "Â§eThe last yellow falls..")
    ...
    ,NICELIFE_NICELIST_VOTE_ERROR_INSUFFICIENT(Seasons.NICE_LIFE, "Â§cInsufficient votes")
    ,NICELIFE_NICELIST_VOTE_ERROR_AGREEMENT(Seasons.NICE_LIFE, "Â§cNo agreement reached")
    ,NICELIFE_NICELIST_VOTE_RESULT(Seasons.NICE_LIFE, "Â§2The winner is...")
    ,LISTS_VOTE_RESULT("Â§2The vote winner is...")
    ...
    */
    ...
    public String getRegisterDefaultValue() {
        if (currentSeason != null && currentSeason.getSeason() == Seasons.LIMITED_LIFE) {
            ...
            else if (this == BOOGEYMAN_MESSAGE) modified = "Â§7You are the Boogeyman. You must by any means necessary kill a Â§2dark greenÂ§7, Â§agreenÂ§7 or Â§eyellowÂ§7 name by direct action to be cured of the curse. If you fail, your time will be dropped to the next color. All loyalties and friendships are removed while you are the Boogeyman.";
            ...
            else if (this == LIVES_RELOADING) modified = "Â§7Reloading times...";
            ...
            else if (this == LIVES_RANDOMIZE_SINGLE) modified = "Â§7Assigning random time to {}Â§7...";
            else if (this == LIVES_RANDOMIZE_MULTIPLE) modified = "Â§7Assigning random time to {}Â§7 targets...";
            ...
            else if (this == LIVES_RANDOMIZE_RESULT) modified = "{}Â§a to live.";
            else if (this == BOOGEYMAN_FAIL) modified = "{}Â§7 failed to kill a player while being the Â§cBoogeymanÂ§7. Their time has been dropped to {}";
